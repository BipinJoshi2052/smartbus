(function ($, baseUrl, constants) {
   ...
   function initPlugins() {
      /* Bootstrap Carousel for Top Banner */
      ...
      $(function () {
         if ($('[data-plugin="cities-ajax"]').length) {
            $('[data-plugin="cities-ajax"]').each(function () {
               getLocations($(this), true);
            });

         }
      });

      /*==== datepicker for departure in front search page ====*/
      $(function () {
         var dateFormat = 'd M, yy',
               ...
               from = $("#departure-date")
                     .datepicker({
                        ...
                        minDate: 0,
                        maxDate: "+" + settings.search_departure_days + "D"
                     })
                     .on("change", function () {
               ...
               to = $("#return-date").datepicker({
                  dateFormat: dateFormat,
                  maxDate: "+" + settings.search_return_days + "D"
               })
                     .on("change", function () {
      ...
      /*==== datepicker for arrival in front search page ====*/
      $(function () {
         /*

               var dep = $('#departure-date').datepicker({
               ...
               var ret = $("#return-date").datepicker({
                  dateFormat: 'DD, d MM, yy',
                  altFormat: "yy-mm-dd",
                  minDate: 0, maxDate: "+1M +10D"
         ...
         */
         ...
         /*   dep.datetimepicker({
           ...
           /* dep.on("dp.change", function (e) {
               ret.data("DateTimePicker").minDate(e.date);
            });
            ret.on("dp.change", function (e) {
               dep.data("DateTimePicker").maxDate(e.date);
            });*/
      });
      /* ==== Filter Functions ==== */
      ...
      if ($('#filters-wrapper').length) {
         $(function () {
            $(function () {
               ...
               if (deviceWidth > 1023) {
                  $('#search-details-wrapper').addClass('with-filter');
               }
            });
            /* == == = toggle pos filter = == ==*/
            $(function () {
               /*==== search filterCollapse ====*/
               $('#search-filter-collapse').on('click', function () {
                  $('#search-details-wrapper').toggleClass('with-filter');
               });
            });
            /* == == = isotope filter Init = == ==*/
            $(function () {
               var $filterable = $('#search-list'),
                     $checkboxes = $('#filters-wrapper input'),
                     $sortBy = $('#sort-tickets .sort');
               ...
               $filterable.isotope({
                  ...
                  getSortData: {
                     rating: function (itemElem) {
                        console.log(itemElem, $(itemElem).data('rating'));
                        return parseInt($(itemElem).data('rating'));
                     },
                     departure: function (itemElem) {
                        return Date.parse($(itemElem).data('departure'));
                     },
                     arrival: function (itemElem) {
                        return Date.parse($(itemElem).data('arrival'));
                     },
                     duration: function (itemElem) {
                        return Date.parse($(itemElem).data('duration'));
                     },
                     seats: function (itemElem) {
                        return $(itemElem).data('seats');
                     },
               ...
               $checkboxes.change(function () {
                  ...
                  // console.log(filters);
                  $filterable.isotope({filter: filters});
               });
               $sortBy.on('click', function () {
                  var sortByValue = $(this).attr('data-sort');
                  $filterable.isotope({sortBy: sortByValue});
               });
      ...
      if (typeof prices === 'object' && $.isEmptyObject(prices) === false) {
         ...
         boarding.find('[type="radio"]').on('change', function () {
            ...
            if (typeof to === 'object' && $.isEmptyObject(to) === false) {
               ...
               // console.log(to);
               dropping.parents('#dropping-stations-list').show();
               dropping.html(d);
   ...
   function toggleBook(modal, price) {
      ...
      if (price > 0) {
         modal.find('#book-btn-wrapper').show();
         modal.find('#book').removeAttr('disabled');
      } else {
         modal.find('#book-btn-wrapper').hide();
