(function ($, baseUrl, constants) {
   ...
   function afterSuccess(modal, schedule) {
      ...
      if (typeof schedule.seats.layout !== "undefined" && !($.isEmptyObject(schedule.seats.layout))) {
         ...
         $.each(schedule.seats.layout, function (index, value) {
            ...
            // var n = ((r !== '') ? ' - ' : '') + value.name;
            var n = ((value.booking !== "undefined" && !($.isEmptyObject(value.booking))) || (value.status != "undefined" && value.status == 0))?value.name:'';
   ...
   function initModal(btn, modal, schedule, content) {
      ...
      modal.find('#seats-container .seat [type="checkbox"]').on("change", function (e) {
         ...
         var length = modal.find('#seats-container .seat [type="checkbox"]:checked').length;


         if (length >= constants['max-seats']) {
      ...
      $.ajax({
         ...
         success: function (response) {
            ...
            if (response.status === true) {
               ...
               bookingDone(schedule, modal, data, response);
            } else {
   ...
   function bookingDone(schedule, modal, data, response) {
      // console.log(schedule);
      // console.log(data);
      modal.find('#company').html(schedule.company);
      modal.find('#code').html(response.booking_code);
      ...
      modal.find('#vehicle-number-plate').html(schedule.vehicle.number_plate);
      modal.find('#contact').html(schedule.vehicle.contact_info);

      modal.find('#ticket-seats').html(response.seat_names);
   ...
   function destroy(btn, content) {
      var booking = $('#booking-panel');
      var booked = $('#booked-panel');
      $('#booking-content').remove();
      booked.hide();
      booking.html(content);
      booking.show();
